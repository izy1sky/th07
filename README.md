...
The first load downloads the ~93 MB asset bundle (`th07.data`), so the
progress bar can take a while to fill.

### Web BGM (MP3 streaming)

The original `thbgm.dat` is 444 MB of raw PCM and does not fit in the
Bilibili Toy 140 MB package limit, so the web build ships BGM as 20 MP3
files (~60 MB total) generated by `tools/convert_bgm_web.py`:

```bash
python tools/convert_bgm_web.py --bitrate 192k
```

The script extracts each track from the original `assets/thbgm.dat`, encodes
it as MP3 into `assets/bgm/`, writes a matching `assets/bgm/thbgm.fmt`
(intro/total lengths stored as PCM frames), and replaces `assets/thbgm.dat`
with a 16-byte stub so the game still enables its WAV/streaming music path.
On Emscripten builds `SoundPlayer` decodes the MP3s with miniaudio and loops
at the original intro/loop points; desktop builds keep the original raw PCM
path unchanged.
