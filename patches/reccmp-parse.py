...
class ParseAsm:
    ...
    def hex_replace_imm(self, match: re.Match) -> str:
        """For replacing immediate value operands (second operand after a comma)."""
        value = int(match.group(1), 16)
        if self.is_addr(value):
            return self.replace(value)
    ...
    def hex_replace_imm_mul(self, match: re.Match) -> str:
        """For multiplication immediates: a multiplier is never a valid address,
        so an OFFSET label here is always a false positive."""
        value = int(match.group(1), 16)
        if self.is_addr(value):
            name = self.lookup(value)
            if name is not None and " (OFFSET)" in name:
                return match.group(0)
            return self.replace(value)

        return match.group(0)

    def hex_replace_imm_bitwise(self, match: re.Match) -> str:
        """For bitwise immediates (and/or/xor/test): a bitmask is never a valid
        address, so an OFFSET label here is always a false positive."""
        value = int(match.group(1), 16)
        if self.is_addr(value):
            name = self.lookup(value)
            if name is not None and " (OFFSET)" in name:
    ...
    def sanitize(self, inst: DisasmLiteInst) -> tuple[str, str]:
        ...
        if (
            ...
            if inst.mnemonic == "push":
                if self.is_addr(op_str_address):
                    name = self.lookup(op_str_address)
                    if name is not None and " (OFFSET)" in name and op_str_address % 4 != 0:
        ...
        if inst.mnemonic == "call":
            ...
            # Also normalize register + displacement indirect calls
            # (e.g. call dword ptr [edx + g_FuncTable+8]).
        ...
        elif inst.mnemonic in ("imul", "mul"):
            op_str = immediate_replace_regex.sub(self.hex_replace_imm_mul, op_str)
        elif inst.mnemonic in ("and", "or", "xor", "test"):
            op_str = immediate_replace_regex.sub(self.hex_replace_imm_bitwise, op_str)
        else:
            op_str = immediate_replace_regex.sub(self.hex_replace_imm, op_str)
